...
\item 已知$ \ln2\approx 0.693147,\ \ln3\approx 1.098612 $，
估算$ \ln 73 $的近似值，要求精确到小数点后两位。\\
\textbf{解}\ 
\begin{align*}
    \ln 73 &=\ln72+\ln\left(1+\frac{1}{72}\right) \\
    & \approx 3\ln 2+2\ln 3+\frac{1}{72} \\
    & \approx 3\times 0.6931+2\times 1.0986+0.0138=4.2903  
\end{align*}
也可以用$ \ln\dfrac{73}{72}=\ln\dfrac{1+\frac{1}{145}}{1-\frac{1}{145}}\approx
\dfrac{2}{145} $. 如果题目只提供了$ \ln 2 $的数值，那么
\begin{align*}
...
\end{align*}
$ \ln 73 $的精确值是$ 4.290459\cdots $. 
如果按照(\ref{lnt近似-泰勒取两项})式计算，结果为$ 2.5600\cdots $，
如果按照(\ref{lnt帕德近似3_3})式计算，结果为$ 3.3674\cdots $，
都差得很远，而且计算量很大。这两个公式只适合估算自变量在$ 1 $附近
的对数值，不能用于估算$ 73 $这种远大于$ 1 $的数的对数值。\\
...
\item (2022，高考全国甲卷)已知$a=\dfrac{31}{32},\ b=\cos\dfrac{1}{4},\ c=4\sin\dfrac{1}{4}$，试比较$a,b,c$的大小. \\
\textbf{方法一}\ 
\begin{align*}
...
\item (2022，新高考I卷)设$a=0.1\e^{0.1},\ b=\dfrac{1}{9},\ c=-\ln0.9 $，
试比较$a,b,c$的大小. \\
...
可惜，是$ x_1^2>a^2-1 $，而不是$ x_1^2<a^2-1 $，否则，把$ \left\{\begin{aligned}
    & x_1^2<a^2-1 \\
    & x_2^2<2(a-1)
\end{aligned}\right. $两式相加就能得到$ x_1^2+x_2^2<a^2+2a-3=(a+3)(a-1) $.
...
让我们考虑如何挽救上面的方法，使之能解决本题。当$ a\to \dfrac{1}{\e} $时，$ x_0\to 1,\ x_1\to 1 $，对$ x_1 $的范围估计所使用的不等式$ x_1<-\ln a $右侧也能给出“1”这个结果，这意味着对$ x_1 $的范围估计基本不能再改进了。而对$ x_0 $的估计$ 1-\ln 2-\dfrac{1}{2}\ln a\to 1.5-\ln 2=0.806852\cdots (a\to \dfrac{1}{\e})$，与“1”的误差较大，还有改进的余地。前面使用了不等式$ x^2\leq \dfrac{4}{\e^2}\e^x $来估计$ x_0 $(因为这个不等式在2016年新课标全国II卷中出现过，容易让人首先想到)，函数$ y=x^2$和$ y=\dfrac{4}{\e^2}\e^x $在$ x=2 $处相等(同时相切)，而在$ x=1 $附近差别较大，如果能找到函数$ A\e^{\alpha x} $与$ x^2 $在$ x=1 $处相等(相切)，那么对$ x_0 $的估计精度就能提高。$ \left\{\begin{aligned}
A\e^{\alpha x}|_{x=1}=&x^2|_{x=1}\ 
   (\text{函数值相等})	\\
A\alpha \e^{\alpha x}|_{x=1}=&2x|_{x=1}\ 
	(\text{导数值相等})
\end{aligned}\right. $，解得$ A=\e^{-2},\ \alpha=2 $，
即$ x^2\leq \e^{2x-2}\ (x>0) $，这个不等式其实可以从最常见的不等式$ \e^x\geq 1+x $衍生而来(把$ x $换成$ x-1 $，再两边平方)。于是$ \dfrac{1}{a}=\underbrace{x_0^2\e^{x_0}< (\e^{2x_0-2})\e^{x_0}}_{x_0>1,\ \text{“=”取不到}}=\e^{3x_0-2}\ \Rightarrow\ -\ln a< 3x_0-2 $，将$ \left\{\begin{aligned}
	 3x_0 & > 2-\ln a	\\
	 -x_1 & >\ln a
\end{aligned}\right. $两式相加即得$ 3x_0-x_1>2 $.
