...
(\ref{正整数k次方求和通式})式实际上是欧拉-麦克劳林求和公式\footnote{可参看《特殊函数概论》第6页。}的一个特例。
...
\begin{align*}
    ...
    =x+x^{2}+\dfrac{x^{3}}{2}+o\left(x^{3}\right)\\
...
Maple、MPFR、GMP等相对冷门的工具。所以，掌握一些基本的转换方法，
使得计算器能派上用场，还是有必要的。

\item \label{光的反射定律} 已知两个不同的定点$ A(x_1,y_1),B(x_2,y_2) $，动点
$P(x,0)$在$x$轴上移动，且$A,B$两点在$x$轴同一侧，
求$ |PA|+|PB| $的最小值。
...
\textbf{注1}：以上面的$ b $为基础，再导出2个数字，下面出现了
23, 127, 52, 1023，这4个数字均来源于IEEE 754浮点数的规范。
\begin{gather*}
    \dfrac{3}{2}\times 2^{23}\times(127-b)=
    1597488310.001508\cdots
\end{gather*} 
最接近上面这个数的整数是1597488310，用16进制表示是5f37bcb6.
\begin{gather*}                
    \dfrac{3}{2}\times 2^{52}\times(1023-b)=
    6910482905085795321.358862\cdots              
\end{gather*}
最接近上面这个数的整数是6910482905085795321，
用16进制表示是5fe6f796c00c5bf9. 
我们将在\ref{无除法的平方根迭代算法}
看到这两个以5f开头的16进制数的作用。\\
\textbf{注2}：设函数 $f(x)$ 在区间$[m,n]$ 上连续\footnote{此处的
$f(x)$已经不再是上文的$f(x)$. }，定义 $f(x)$ 与直线 $g(x)=ax+b$ 的偏差为：
\begin{gather*}
    E = \max_{m \leq x \leq n} |f(x) - (ax+b)|.
\end{gather*}
记所有一次函数构成的集合为：
\begin{gather*}
    A = \{ g(x) = ax+b \mid k \in \mathbb{R},\ b \in \mathbb{R} \}. 
\end{gather*}
若存在 $g_0(x) \in A$，使得对任意 $g(x) \in A$，恒有
\begin{align*}
    \max_{m \leq x \leq n} |f(x) - g_0(x)| &\leq \max_{m \leq x \leq n} |f(x) - g(x)|,
\end{align*}
则称 $g_0(x)$ 为 $f(x)$ 在切比雪夫意义下的最佳逼近直线。上式可改写为：
\begin{gather*}
    \max_{m \leq x \leq n} |f(x) - g_0(x)| = \min_{g \in A} \max_{m \leq x \leq n} |f(x) - g(x)|.
\end{gather*}
若 $f$ 在 $[m,n]$ 上二阶可导，且 $f''(x)$ 在 $[m,n]$ 上不变号，则 $f(x)$ 的最佳逼近直线为：
\begin{align}\label{切比雪夫最佳逼近直线方程}
    g_0(x) = \frac{f(n)-f(m)}{n-m} \left( x - \frac{m+c}{2} \right) + \frac{f(m)+f(c)}{2}
\end{align}
其中 $c \in [m,n]$ 满足$f'(c) = \dfrac{f(n)-f(m)}{n-m}$.
...
\begin{align*}
    ...
    = \left(\dfrac{1+C^2a^2\e^{2ax}}{C^{2/3}a^{4/3}\e^{2ax/3}}
          \right)^{3/2} \\
...
(2) \textbf{方法一}
\begin{align*}
    f(x)=[x(x+3)][(x+1)(x+2)]=&(x^2+3x)(x^2+3x+2) \tag*{\ding{172}}\\
    =&[(x^2+3x)+1]^2-1 \geq -1 
\end{align*}
当$ x^2+3x+1=0 $，即 $ x=\dfrac{-3\pm\sqrt{5}}{2} $时，等号成立。\\
\ding{172}式中的因式分解的技巧性较强，事实上有更一般的
...
$ A. \Big(\dfrac{1}{4},\dfrac{1}{2}\Big) \hspace{2cm} 
  B. \Big(\dfrac{1}{16},\dfrac{1}{8}\Big) \hspace{2cm} 
  C. \Big(\dfrac{1}{3},\dfrac{1}{2}\Big)\hspace{2cm} 
  D. \Big(\dfrac{1}{8},\dfrac{1}{4}\Big)  $ \medskip \\
...
当$ c=\pm \dfrac{1}{4} $时，恰好有一个极值点落在$ x $轴上，此时极值点成为二重实根，即，
\begin{gather}\label{2020高考存在重根的三次方程}
 f(x)=\left(x-\dfrac{1}{2} \right)^2(x+1)  
 \q \text{或} \q f(x)=\left(x+\dfrac{1}{2} \right)^2(x-1) 
\end{gather}
...
$ \begin{dcases}
    u^3-\dfrac{3}{4}u-\dfrac{1}{4}=
            (u-1)\Big(u+\dfrac{1}{2}\Big)^2 \leq 0 \\
    u^3-\dfrac{3}{4}u+\dfrac{1}{4}=
            (u+1)\Big(u-\dfrac{1}{2}\Big)^2 \geq 0 
...
因为$ \cos3\theta=\cos3\Big(\theta+\dfrac{2\pi}{3}\Big)=
\cos3\Big(\theta+\dfrac{4\pi}{3}\Big) $，
所以$ x_2=\cos(\theta+\dfrac{2\pi}{3}),x_3=\cos(\theta+\dfrac{4\pi}{3}) $也是方程的根，
这两个根的绝对值显然小于等于1.  而当$\theta=k\pi\, (k\in\textbf{Z})$时，$x_2=x_3=\pm \dfrac{1}{2}$，
那么$x_2,x_3$是否必然是原方程的一对重根呢？(比如$x_1$是第一个根，
$x_2,x_3$都是第二个根，而第3个根没有找到。)其实是重根，因为
$x_1=\cos\theta=\cos(k\pi)=\pm 1,\ c=\pm\dfrac{1}{4}$，
原方程就是\ref{2020高考存在重根的三次方程}式，
$x_2=x_3=\pm \dfrac{1}{2}$就是一对重根。\\
...
可惜，是$ x_1^2>a^2-1 $，而不是$ x_1^2<a^2-1 $，否则，把$ \begin{dcases}
    x_1^2<a^2-1 \\
    x_2^2<2(a-1)
\end{dcases}$两式相加就能得到$ x_1^2+x_2^2<a^2+2a-3=(a+3)(a-1) $.
...
\begin{dcases}
    ...
    \ln x_3=x_2
\end{dcases}$. \\
再结合\ding{172}式，$ x_1+x_3=\e^{x_1}+\ln x_3=x_2+x_2 $，证毕。
...
\textbf{解}\ (I)略 \\
(II) \textbf{方法一} $ f'(x)=-\dfrac{1}{x^2}-1+\dfrac{a}{x}=-\dfrac{1}{x^2}(x^2-ax+1) $，令$ x^2-ax+1=0,\ \Delta=a^2-4>0 $，由韦达定理，$ x_1+x_2=a>0,\ x_1x_2=1 $，所以$ a>2 $. 不妨设$ 0<x_1<1<x_2 $，又有
\begin{align*}
...
\end{align*}
根据(\ref{不等式2lnt<t-1/t})式，(当然可以直接求导证明)，有$ 0<2\ln x_2<x_2-\dfrac{1}{x_2} $，所以$ \dfrac{2a\ln x_2}{x_2-\frac{1}{x_2}}<a $，原不等式成立。
...
\textbf{方法二}(由董琳提供) 同样有 \(x_1x_2 = 1\)，故 \(x_2 = \frac{1}{x_1}\)，由对称性，
不妨设$x_1 > 1$，则 \(f(x_2)=f(\frac{1}{x_1})=-f(x_1)\)，且 \(f(1) = 0\).
\begin{gather*}
    \frac{f(x_1)-f(x_2)}{x_1-x_2} 
    = \frac{2 f(x_1)}{x_1-\frac{1}{x_1}}
    = \frac{2x_1 f(x_1)}{x_1^2 - 1} 
    = \frac{2x_1 f(x_1) - 2\cdot 1 \cdot f(1)}{x_1^2 - 1^2}
\end{gather*}
令 \(F(x) = 2x f(x)\)，\(G(x) = x^2\)，则 \(F(1) = 0\)，\(G(1) = 1\).
\begin{align*}
    F'(x) =&\ 2\left[f(x) + x f'(x)\right]=2[-2x+a(\ln x+1)] \\
    G'(x) =&\ 2x
\end{align*}
由柯西中值定理\footnote{\(F(x),G(x)\) 在 \([1,x_1]\) 连续，
    \((1,x_1)\) 可导，且 \(G'(x)=2x \neq 0\)}，
存在 \(\xi \in (1, x_1)\)，使得
\begin{gather*}
    \frac{F(x_1) - F(1)}{G(x_1) - G(1)} = \frac{F'(\xi)}{G'(\xi)}
    =\frac{-2\xi+a(\ln \xi+1)}{\xi}=-2+\frac{\ln \xi + 1}{\xi}a
\end{gather*}
因为$\xi >1 $，所以
\begin{gather*}
    \ln \xi + 1 < \xi \implies \frac{\ln \xi + 1}{\xi} < 1    \\
    \frac{f(x_1)-f(x_2)}{x_1-x_2} = \frac{\ln \xi + 1}{\xi}a - 2 < a - 2 
\end{gather*}
...
% XinGaoKao_I_2021.m
\item\label{2021新高考全国I卷} (2021，新高考全国I卷)已知函数$ f(x)=x(1-\ln x)\ (x>0) $ \\
(1) 讨论$ f(x) $的单调性；\\
...
\begin{align}
    ...
    \e^2\q (\text{消去}\ x_0) \tag*{\ding{172}}\\
    & =a^{x_0}-(a^{x_0}\ln a)x_0+\e^2 \q (\text{消去}\ b) 
    \tag*{\ding{173}}
\end{align}
\textbf{方法一}\ 因为$ a>1 $，所以\ding{172}式等价于
\begin{gather}
    b -b\ln b+b\ln(\ln a)+\e^2\ln a<0 \tag*{\ding{174}}
\end{gather}
对任意$ b>2\e^2 $恒成立，将上式左侧定义为$ g(b) $，
则$ g'(b)=-\ln b+\ln(\ln a) $. \\
(一) 若$ \ln a>2\e^2 $，则$ g'(\ln a)=0 $，
$ g(b) $在$ b=\ln a $时取得极大值，但$ g(\ln a)=(1+\e^2)\ln a>0 $，
不符合\ding{174}式。\\
(二) $ \ln a\leq 2\e^2 $，则$ g'(b)<0 $，
...
\begin{gather*}
    ...
    \tag*{\ding{176}}
\end{gather*}
将\ding{176}式右侧关于$ b $的函数记为$ \varphi(b) $，
...
巧合的是，这一题与\ref{2021新高考全国I卷}一样，都出现了函数$ y= x-x\ln x $. 不知道命题人是有意还是无意，函数$ y= x-x\ln x $可是大有来头：与黎曼猜想有关！黎曼zeta函数的所有非平凡零点(或者说是复数零点)的实部都是$ \dfrac{1}{2} $(尚未证明)，
...
\item (2024，天津高考)	定义函数$f(x)=x\ln x$.\\
(I) 求$f(x)$图像上点$(1,f(1))$处的切线方程；\\
(II) 若$f(x)\geq a(x-\sqrt{x})$在$x\in(0,\infty)$时恒成立，
求$a$的取值范围；\\
(III) 若$x_1,x_2\in(0,1)$，证明$|f(x_1)-f(x_2)|\leq|x_1-x_2|^{\frac{1}{2}}$. \\
\textbf{解}：(I) $y=x-1$. \\
(II) 在$ x\ln x\geq a(x-\sqrt{x})$两边同时除以$x$，移项，
有$\ln x-a+\dfrac{a}{\sqrt{x}}\geq 0$.  
令$F(x)=\ln x-a+\dfrac{a}{\sqrt{x}}$，则
$F'(x)=\dfrac{1}{x}\left(1-\dfrac{a}{2\sqrt{x}}\right)$.\\
若$a\leq 0$，则$F(x)$在$(0,\infty)$上单调递增，
但$F(1)=0$，那么当$x\in(0,1)$时，$F(x)<0$，不符合题意，
所以$a>0$. $F'(x)=0$的根为$x=\dfrac{a^2}{4}$，
$F(x)$在此处取得极小值，那么要求
\begin{gather*}
    F\left(\dfrac{a^2}{4}\right)=
    2\left[\ln\dfrac{a}{2}-\left(\dfrac{a}{2}-1\right)\right] \geq 0
\end{gather*}
但是已知$ \ln\dfrac{a}{2}\leq \dfrac{a}{2}-1\ 
\left(\dfrac{a}{2}>0\right) $，所以必有
$\ln\dfrac{a}{2}=\dfrac{a}{2}-1$，$a=2$. \\\\
\textbf{注}：实际上，由罗必塔法则，$\lim\limits_{x\to 1}
\dfrac{x\ln x}{x-\sqrt{x}}=\lim\limits_{x\to 1}
\dfrac{1+\ln x}{1-\frac{1}{2\sqrt{x}}}=2$. \\
(III)\textbf{方法一}(由董琳提供) \(x_1, x_2\) 地位平等，不妨设
\(0 < x_1 \leq x_2 < 1\). 原不等式等价于
\begin{align}\label{2024天津不等式去掉绝对值}
    -\sqrt{x_2 - x_1} \leq f(x_1) - f(x_2) \leq \sqrt{x_2 - x_1}. 
\end{align}
先证\eqref{2024天津不等式去掉绝对值}式左半部分。\\
对于 \(0 < x_1 \leq x_2 < 1\)，有 \(0 \geq x_1 - x_2 \geq -\sqrt{x_2 - x_1} > -1\)，
故只需证更强的不等式：
\begin{align*}
    f(x_1) - f(x_2) \geq & x_1 - x_2   \\
    f(x_1) - x_1    \geq & f(x_2) - x_2  
\end{align*}
令$ g(x) = f(x) - x=x\ln x-x,\ x \in (0,1) $，则$g'(x)=\ln x < 0$.
因此 \(g(x)\) 在 \((0,1)\) 上单调递减，故$g(x_1) \geq g(x_2)$，
\eqref{2024天津不等式去掉绝对值}式左半部分成立。\\
再证\eqref{2024天津不等式去掉绝对值}式右半部分。\\
对于 \(0 < x_1 \leq x_2 < 1\)，有 \(\sqrt{x_2} - \sqrt{x_1} \leq \sqrt{x_2 - x_1}\)\ (两边平方即可证明)，故只需证：
\begin{align*}
    f(x_1) - f(x_2)     \leq & \sqrt{x_2} - \sqrt{x_1}, \\
    f(x_1) + \sqrt{x_1} \leq & f(x_2) + \sqrt{x_2}. 
\end{align*}
定义$h(x) = f(x) + \sqrt{x}, \ x \in (0,1)$，
利用(II)中的结论：\(\ln x \geq 2 \left(1 - \dfrac{1}{\sqrt{x}}
\right) \ (x>0)\)，有
\begin{align*}
    h'(x) = 1 + \ln x + \frac{1}{2\sqrt{x}} 
    =&\ \ln(\e^3 x) - 2 + \frac{1}{2\sqrt{x}} \\
    \ge &\ 2\left(1-\frac{1}{\sqrt{e^3x}}\right)-2+
    \frac{1}{2\sqrt{x}} 
    =\ \left(\frac{1}{2}-\frac{2}{\sqrt{e^3}}\right)
    \frac{1}{\sqrt{x}}>0
\end{align*}
其中利用了$\e^3=20.085\cdots>4^2$，所以
\(h(x)\) 在 \((0,1)\) 上单调递增，则$h(x_1) \leq h(x_2)$，
\eqref{2024天津不等式去掉绝对值}式右半部分成立。\\
综上，原命题得证。\\
\textbf{方法二}(由董琳提供) 不妨令 $0 < x_1 < x_2 < 1$，由牛顿-莱布尼茨公式，有
\begin{gather*}
    f(x_2) - f(x_1) = \int_{x_1}^{x_2} f'(t) \, \d t=
    \int_{x_1}^{x_2} (1 + \ln t) \, \d t
\end{gather*}
上式两边取绝对值得：
\begin{gather*}
    |f(x_2) - f(x_1)| = \left| \int_{x_1}^{x_2} (1 + \ln t) \, \d t \right|.
\end{gather*}

由赫尔德(Hölder)不等式(积分形式)：
\begin{gather}
    \left| \int_a^b u(t) v(t) \, \d t \right| \leq \left( \int_a^b |u(t)|^p \, \d t \right)^{\frac{1}{p}} \left( \int_a^b |v(t)|^q \, \d t \right)^{\frac{1}{q}}, \label{eq:holder_ineq}
\end{gather}
其中参数满足共轭关系 $\frac{1}{p} + \frac{1}{q} = 1$.这里取 $p = q = 2$，令 $u(t) = 1 + \ln t$，$v(t) = 1$，代入 \eqref{eq:holder_ineq} 得：
\begin{align*}
    \left| \int_{x_1}^{x_2} (1 + \ln t) \cdot 1 \, \d t \right|
    &\leq \left( \int_{x_1}^{x_2} |1 + \ln t|^2 \, \d t \right)^{\frac{1}{2}} \cdot \left( \int_{x_1}^{x_2} 1^2 \, \d t \right)^{\frac{1}{2}}.
\end{align*}
上式不等号右侧第二项的值为：
\begin{gather*}
    \left( \int_{x_1}^{x_2} 1^2 \, \d t \right)^{\frac{1}{2}} = 
    (x_2 - x_1)^{\frac{1}{2}}.
\end{gather*}

对于右侧第一项，由 $|1 + \ln t|^2 \ge 0$ 得：
\begin{gather*}
    \int_{x_1}^{x_2} |1 + \ln t|^2 \, \d t \leq \int_{0}^{1} |1 + \ln t|^2 \, \d t =\left.(t+t\ln^2 t) \right|_0^1=1.
\end{gather*}
上式利用了极限$\lim\limits_{t\to 0} t\ln^2 t =0 $.
综上，可得最终结论：
\begin{align*}
    |f(x_2) - f(x_1)|
    &\leq \left( \int_{x_1}^{x_2} |1 + \ln t|^2 \, \d t \right)^{\frac{1}{2}} \cdot (x_2 - x_1)^{\frac{1}{2}} \\
    &\leq 1 \cdot |x_2 - x_1|^{\frac{1}{2}} \\
    &= |x_2 - x_1|^{\frac{1}{2}}.
\end{align*}
...
\begin{align*}
    ...
    2M&\geq 2\left|f\Big(-\dfrac{1}{2}\Big)\right|=
    2\left|-\dfrac{1}{8}+\dfrac{1}{2}a-b\right|=
    \left|\dfrac{1}{4}-a+2b\right| \\
    2M&\geq 2\left|f\Big( \dfrac{1}{2}\Big)\right|=
    2\left| \dfrac{1}{8}-\dfrac{1}{2}a-b\right|=
    \left|\dfrac{1}{4}-a-2b\right|
\end{align*}
...
\begin{gather*}
    6M\geq \left|(-1+a-b)+(-1+a+b)+
    \Big(\dfrac{1}{4}-a+2b\Big)+\Big(\dfrac{1}{4}-a-2b\Big)\right|
    =\dfrac{3}{2}
\end{gather*}
...
换成$ \pm \dfrac{1}{3} $，那么
\begin{align*}
    3M&\geq 3\left|f(-\dfrac{1}{3})\right|=
    3\left|-\dfrac{1}{27}+\dfrac{1}{3}a-b\right|=
    \left|\dfrac{1}{9}-a+3b\right| \\
    3M&\geq 3\left|f\Big(\dfrac{1}{3}\Big)\right|=
    3\left| \dfrac{1}{27}-\dfrac{1}{3}a-b\right|=
    \left|\dfrac{1}{9}-a-3b\right|  
\end{align*}
同样取4个式子相加，
\begin{align*}
    8M\geq \left|(-1+a-b)+(-1+a+b)+\Big(\dfrac{1}{9}-a+3b\Big)+
    \Big(\dfrac{1}{9}-a-3b\Big)\right|=\dfrac{16}{9}
\end{align*}
...
x_0)\cup(x_0,2] $，满足$ \left|\dfrac{p}{q}-x_0\right|\geq 
 \dfrac{1}{Aq^4} $. \\
\textbf{解}\ (1)\ 以下一律使用$ f'(x) $代替$ g(x) $，能让思路更加清晰。
$ f'(x)=8x^3+9x^2-6x-6 $，$ f''(x)=6(4x^2+3x-1)=6(4x-1)(x+1) $，
所以$ f'(x) $在$ (-\infty,-1) $和$ \Big(\dfrac{1}{4},+\infty\Big) $上分别单调递增，
在$ \Big(-1,\dfrac{1}{4}\Big) $上单调递减。\\
...
\end{gather}
因为$ f'(x) $在$ \Big(\dfrac{1}{4},+\infty\Big) $上单调递增，
所以上式显然成立。
...
让我们考虑如何挽救上面的方法，使之能解决本题。当$ a\to \dfrac{1}{\e} $时，$ x_0\to 1,\ x_1\to 1 $，对$ x_1 $的范围估计所使用的不等式$ x_1<-\ln a $右侧也能给出“1”这个结果，这意味着对$ x_1 $的范围估计基本不能再改进了。而对$ x_0 $的估计$ 1-\ln 2-\dfrac{1}{2}\ln a\to 1.5-\ln 2=0.806852\cdots (a\to \dfrac{1}{\e})$，与“1”的误差较大，还有改进的余地。前面使用了不等式$ x^2\leq \dfrac{4}{\e^2}\e^x $来估计$ x_0 $(因为这个不等式在2016年新课标全国II卷中出现过，容易让人首先想到)，函数$ y=x^2$和$ y=\dfrac{4}{\e^2}\e^x $在$ x=2 $处相等(同时相切)，而在$ x=1 $附近差别较大，如果能找到函数$ A\e^{\alpha x} $与$ x^2 $在$ x=1 $处相等(相切)，那么对$ x_0 $的估计精度就能提高。$ \begin{dcases}
A\e^{\alpha x}|_{x=1}=&x^2|_{x=1}\ 
   (\text{函数值相等})	\\
A\alpha \e^{\alpha x}|_{x=1}=&2x|_{x=1}\ 
	(\text{导数值相等})
\end{dcases}$，解得$ A=\e^{-2},\ \alpha=2 $，
即$ x^2\leq \e^{2x-2}\ (x>0) $，这个不等式其实可以从最常见的不等式$ \e^x\geq 1+x $衍生而来(把$ x $换成$ x-1 $，再两边平方)。于是$ \dfrac{1}{a}=\underbrace{x_0^2\e^{x_0}< (\e^{2x_0-2})\e^{x_0}}_{x_0>1,\ \text{“=”取不到}}=\e^{3x_0-2}\ \Rightarrow\ -\ln a< 3x_0-2 $，将$ \begin{dcases}
	 3x_0 > 2-\ln a	\\
	 -x_1 >\ln a
\end{dcases}$两式相加即得$ 3x_0-x_1>2 $.
...
\begin{gather*}
    ...
    \dfrac{c}{\sqrt{c^2+a^2}}\leq \dfrac{3\sqrt{2}}{2}  \tag*{\ding{172}}
\end{gather*}
而这个不等式又是起源于2001年的第42届IMO试题：对任意正实数$ a,b,c $，求证：
\begin{gather*}
    \dfrac{a}{\sqrt{a^2+8bc}}+\dfrac{b}{\sqrt{b^2+8ca}}+
    \dfrac{c}{\sqrt{c^2+8ab}}\geq 1  \tag*{\ding{173}}
\end{gather*}
\ding{172}式等价于
\begin{gather*}
...
这与$ f(x)=\dfrac{1}{\sqrt{1+x}}+\dfrac{1}{\sqrt{1+a}}+\sqrt{\dfrac{1}{1+
\frac{8}{ax}}} $几乎一样(因为$a\cdot x \cdot \dfrac{8}{ax}=8 $). \\
...
\begin{align}
    \Delta=(24a-64)^2-32a^3=-32(a-2)(a-8)^2 \tag*{\ding{174}}
\end{align}
第(1)问中取$ a=8 $，比较简单。
但对于第(2)问，并不能确定判别式\ding{174}的正负号，二次方程未必有实根。
即使有实根，表达式也过于复杂，无法继续计算。所以，必须转变思路，另辟蹊径。\\
\\
先证$ f(x)>1 $. \\
\textbf{方法一}\ 判别式\ding{174}已经提示我们，$ a=2 $是一个关键的分界点，
...
\textbf{方法三}\ 设$ b=\dfrac{8}{ax} $，$ X=\dfrac{1}{\sqrt{1+x}}<1,
\ A=\dfrac{1}{\sqrt{1+a}}<1,\ B=\dfrac{1}{\sqrt{1+b}}<1 $，那么
\begin{align*}
...
(参看本书第\pageref{Lucas定理-凸包}页)：多项式的导函数的零点全都落在多项式
本身的零点所形成的凸包之内。
...
\end{align*}
\\
(II)\textbf{方法一} 容易证明数列$ \{x_n\} $是单调递减的，因为
\begin{align*}
...
\textbf{方法二}(由数海居士提供)\(n\leq3\)的情况显然成立，当\(n\geq4,x\geq\dfrac{9}{10}\)时，\(f_n(x)\geq f_n \left(\dfrac{9}{10}\right)\geq f_4\left(\dfrac{9}{10}\right)>0\).
所以，\(n\geq4\)时，\(x_n\leq\dfrac{9}{10}\). 令
\begin{gather*}
    g_n(x)=\dfrac{1}{n}f_n'(x)=
    \dfrac{1}{n}\left(1+\dfrac{x}{2}+\dfrac{x^2}{3}+\cdots+
    \dfrac{x^{n-1}}{n}\right)
\end{gather*}
我们先证明，当\(x\in \left[\dfrac{2}{3},\dfrac{9}{10}\right]\)
且\(n\geq4\)时，
\begin{gather}\label{fnx+gnx不等式}
    f_n(x)+g_n(x)>f_{n+1}(x)+g_{n+1}(x)
\end{gather}
上式等价于
\begin{align*}
    \dfrac{1}{n}\left(1+\dfrac{x}{2}+\dfrac{x^2}{3}+\cdots
    \dfrac{x^{n-1}}{n}\right)>
    \dfrac{1}{n+1}\left(1+\dfrac{x}{2}+\dfrac{x^2}{3}+\cdots
    \dfrac{x^n}{n+1}\right)+\dfrac{x^{n+1}}{(n+1)^2}
\end{align*}
等价于
\begin{align*}
    \left(1+\dfrac{1}{n}\right)\left(1+\dfrac{x}{2}+\dfrac{x^2}{3}+\cdots
    +\dfrac{x^{n-1}}{n}\right)>x^n+x^{n+1}
\end{align*}
上式左侧大于\(1+\dfrac{x}{2}+\dfrac{x^2}{3}+\dfrac{x^3}{4}\geq\dfrac{14}{9}\ \left(x=\dfrac{2}{3}\text{时的值}\right)>\left(\dfrac{9}{10}\right)^4+\left(\dfrac{9}{10}\right)^5=\dfrac{124659}{100000}\geq x^n+x^{n+1} \).
所以\eqref{fnx+gnx不等式}式成立。当\(n\geq4\)时展开\(f_n\left(x_{n+p}+\dfrac{1}{n}\right)\)可知：
\begin{gather*}
    f_n\left(x_{n+p}+\dfrac{1}{n}\right)>f_n(x_{n+p})+g_n(x_{n+p})>
    f_{n+p}(x_{n+p})+g_{n+p}(x_{n+p})\geq 0=f_n(x_n)
\end{gather*}
根据\(f_n(x)\)单调递增可知\(x_{n+p}+\dfrac{1}{n}>x_n\). \\
...
\begin{align*}
    f_n\left(\dfrac{5}{7}\right)< &\ -1+\dfrac{5}{7}+
    \dfrac{\Big(\dfrac{5}{7}\Big)^2}{2^2}
...
\begin{align*}
    f_n\left(\dfrac{5}{7}\right)< &\ -1+\dfrac{5}{7}+\dfrac{
        \Big(\dfrac{5}{7}\Big)^2}{2^2}+\dfrac{1}{3^2}\left[\left(\dfrac{5}{7}\right)^3+
...
\item 求证：当$x>\dfrac{1}{24}$时，$\ln\Big(1+\dfrac{1}{12x-0.5}\Big)-
\dfrac{1}{12x}>0$. 
...
\item 求证：当$ x<\dfrac{1}{2} $时，$ \e^x\leq 
 \dfrac{1}{\Big(1-\dfrac{x}{2}\Big)^2}
\leq \dfrac{1}{1-x}\leq \sqrt{\dfrac{1}{1-2x}} $.
(均为$ \e^x\geq 1+x $的衍生不等式。)
...
\item (2024，新高考一卷) 已知函数$f(x)=\ln \dfrac{x}{2-x}+ax+b(x-1)^3$. \\
(I)  若$b=0$，且$f'(x)\geq 0$，求$a$的最小值；\\
(II)  证明：曲线$y=f(x)$是中心对称图形；\\
(III)  若$f(x)>-2$，当且仅当$1<x<2$，求$b$的取值范围。
...
\end{dcases}$，满足$a^a=b^b$，相比直接验证，
改为验证$a\ln a=b\ln b$会更加容易。 

\item 当$ x\in\left(\dfrac{1}{2},1\right) $时，
$ \dfrac{\ln x}{\ln(1-x)}>\left(\dfrac{1-x}{x}\right)^{\frac{1}{\ln 2}} $. 
\item 当$ x>0 $时，$ x^{\frac{1}{x}}\ln x<\sqrt{x} $. 
(\textbf{注}：$ x^{\frac{1}{x}}\ln x-\sqrt{x} $的极大值为$ -0.015439\cdots $) 
\item 当$ x>0 $时，$ x^{\frac{1}{x}}\ln x >-\dfrac{2}{9} $.
(\textbf{注}：$ x^{\frac{1}{x}}\ln x $的极小值为$ -0.222218\cdots $，
与$ -\dfrac{2}{9}=-0.222222\cdots $十分接近。) 
\item 当$ 0<|x|<4.7 $时，$ \left(\dfrac{\sin x}{x}\right)^3>\cos x$.
\item 当$ x>1.2 $时，$ \sin\sqrt{\dfrac{2}{x}}>\sqrt{\dfrac{2}{x+1}} $. 
\item 当$0<x<\dfrac{\pi}{2}$时，$\sin(\sqrt{x})<\sqrt{\sin x}$. 
\end{enumerate}
...
\item 根据光的反射定律\ref{光的反射定律}，\\
$ f(x)=\sqrt{(x-0)^2+3^2}+\sqrt{(x-2)^2+4^2}\geq 
\sqrt{(2-0)^2+(4+3)^2} =\sqrt{53} $.

\item 令$f(x)=\ln\Big(1+\dfrac{1}{12x-0.5}\Big)-\dfrac{1}{12x}$，则
$f'(x)=-\dfrac{1}{12x^2(24x-1)(24x+1)}$，当$x>\dfrac{1}{24}$时，
$f'(x)<0$，$f(x)$单调递减。因为$\lim\limits_{x\to+\infty}f(x)=0$，
所以当$x>\dfrac{1}{24}$时，$f(x)>0$.
...
\begin{gather*}
    ...
    \Big(p\dfrac{x_1}{x_2}+1-p\Big)\geq 0 \tag*{\ding{172}}
\end{gather*}
...
\begin{align*}
    ...
    1+p\Big(\dfrac{x_1}{x_2}-1\Big) \tag*{\ding{173}}
\end{align*}
由伯努利不等式，\ding{173}式显然成立。
...
当$n\geq 2$时，只需证明
\begin{align}
{\small  \left(n+\dfrac{1}{2}\right)^{1/2}
...
\end{gather*}
最后一式显然成立，以上步骤步步可逆，于是\ding{172}式成立，
在\ding{172}式中依次取$n=2,3,4,\cdots$，再结合
$n=1$时的不等式，累加即可。
...
综上，实数$ k $的最大值是2. \\
\textbf{注}：此题的背景显然是\eqref{ln(1+t)_(1-t)幂级数}式：
\begin{align*}
    \ln\dfrac{1+x}{1-x}=2\left(x+\dfrac{x^3}{3}+\dfrac{x^5}{5}+\cdots +\dfrac{x^{2k-1}}{2k-1} +\cdots \right)
\end{align*}

\item \textbf{解}：(I) $a$的最小值是-2. \\
(II) \(f(2-x)+f(x)=2a\)，对称中心为\((1,a)\). \\
(III) 令 \( g(x) = f(x) + 2 \)，由题意可得，当 \( 1 < x < 2 \) 时，有 \( g(x) > 0 \) 恒成立，且  
\( g(1) \leq 0 \). 若 \( g(1) < 0 \)，又 \( g\left(\dfrac{3}{2}\right) > 0 \)，则由零点存在定理可知，存在 \( x_0 \in \left(1, \dfrac{3}{2}\right) \)，使 \( g(x_0) = 0 \)，  
这与题意矛盾，故 \( g(1) = 0=f(1) + 2 = a + 2 = 0 \)，即 \( a = -2 \). 又有
\begin{gather*}
    f'(x) = \frac{2(x-1)^2}{x(2-x)} + 3b(x-1)^2 = (x-1)^2 \left[ \frac{2}{x(2-x)} + 3b \right] 
\end{gather*}
若 \( b < -\dfrac{2}{3} \)，当 \( x \in \left(1, 1+\sqrt{1+\frac{2}{3b}}\right) \) 时，有  
$\dfrac{2}{x(2-x)} + 3b < 0$，即 \( f'(x) < 0 \)，
此时 \( f(x) \) 在区间 \((1, 1+\sqrt{1+\frac{2}{3b}})\) 单调递减，不符合题意；\\
若 \( b \geq -\dfrac{2}{3} \)，当 \( x \in (0, 2) \) 时，
有
\begin{gather*}
    \dfrac{2}{x(2-x)} + 3b\geq \dfrac{2}{\left[
        \frac{x+(2-x)}{2}\right]^2}+3b=2+3b \geq 0
\end{gather*}
此时 \( f(x) \) 在区间 \((0, 2)\) 单调递增，符合题意。\\
故 \( b \) 的取值范围是 \([-\frac{2}{3}, +\infty)\). \\	
(以下由董琳提供)
将 $g(x)$ 向左平移1个单位得到$h(x)$，即
\begin{gather*}
    h(x) = \ln\frac{x+1}{1-x} - 2x + bx^3.
\end{gather*}
原命题 $f(x) \ge -2 \ (1<x<2)$ 等价于：
\[ h(x) \ge 0 \quad (0<x<1) \]
利用\eqref{ln(1+t)_(1-t)幂级数}式有：
\begin{gather*}
    \ln\frac{x+1}{1-x} = 2x + \frac{2}{3}x^3 + o(x^3)
\end{gather*}
代入 $h(x)$ 得：
\begin{align*}
    h(x) \ge 2x + \frac{2}{3}x^3 - 2x + bx^3 
    = \left( \frac{2}{3} + b \right) x^3 \geq 0.
\end{align*}
要使上式在$(0,1)$成立，则：$b \geq -\frac{2}{3}$.
...
\begin{gather}
    x^2f''(x)+4xf'(x)+2f(x)=\dfrac{x-1}{x^2}\e^x \tag*{\ding{172}} \\
    x^2f'''(x)+6xf''(x)+6f'(x)=\dfrac{x^2-2x+2}{x^3}\e^x  
    \tag*{\ding{173}}
\end{gather}
在\ding{172}式中令$ x=2 $，可得$ f''(2)=0 $. 
在\ding{173}式中令$ x=2 $，可得$ f'''(2)=\dfrac{\e^2}{16} $，
...
如果
\begin{gather}
    \dfrac{\e}{\e+1}\ln a\leq \dfrac{r}{r+1}
    \Big(\ln\dfrac{\e}{r}+\ln a\Big)  \tag*{\ding{172}}
\end{gather}
成立，那么
\begin{gather*}
...
\end{gather*}
即原不等式右半部分得证。把\ding{172}式左右两侧看成关于
...
\item 
(7) 参见 https://zhuanlan.zhihu.com/p/1926420751084417604
