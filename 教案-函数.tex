...
\item $ y=\dfrac{Ax+B}{Cx+D} $的对称中心为$\left(-\dfrac{D}{C},
\dfrac{A}{C}\right)$，对称中心的横坐标就是分母的零点，
纵坐标就是$x$趋近于无穷大时函数值的极限。
根据$x$的范围(该范围通常比$\Big(-\infty,-\dfrac{D}{C}\Big)\cup
\Big(-\dfrac{D}{C},+\infty\Big)$更小)求值域的方法如下：
...
% ABS_Plot.m
\item 函数$ y=|x-b_1|+|x-b_2| $的最小值在区间$ [b_1,b_2] $内取得
(假设$ b_1<b_2 $)，在此区间内，函数值为常数$ b_2-b_1 $. 
函数$ y=|x-b_1|+|x-b_2|+|x-b_3| $的最小值在
$ x=b_2 $处取得(假设$ b_1<b_2<b_3 $)，最小值为$ b_3-b_1 $. 
更一般地，对于$ y=|x-b_1|+|x-b_2|+\cdots+|x-b_n| $，假设
$ b_1<b_2<\cdots<b_n $，如果$ n $是偶数，那么最小值在区间
$ [b_{\frac{n}{2}},b_{\frac{n}{2}+1}] $内取得；
如果$ n $是奇数，那么最小值在$ x=b_{\frac{n+1}{2}} $处取得。
一些例子见图\ref{多个绝对值求和图像}. 这一类函数是有实际用途的，
比如电脑机房的路由器该布置在什么位置？才能使从路由器到所有电脑的
网线长度之和最短(假设只有一台路由器，路由器和每台电脑之间都连有
一根网线，电脑是按矩形阵列排布，网线不能斜着拉，只能沿$x,y$两个方向拉)。
结论自然就是要把路由器摆在所有电脑的中心位置。
...
\begin{align*}
    ...
    f(x_1x_2)=&\ \lambda f(x_1)f(x_2)   	 
                        &  f(x)=\dfrac{1}{\lambda} x^{\alpha} \\
    f(x_1+x_2)=&\ \lambda f(x_1)f(x_2)   
                        & f(x)=\dfrac{1}{\lambda} a^x \\
...
如果函数方程变成$ f(x_1+x_2)=f(x_1)+f(x_2)+C $，那么解变成
$f(x)=kx-C$. 
...
\textbf{注}：我认为题目中的不等式在数学研究和工程技术领域都不太可能出现，
是数学教育中的糟粕。
...
\end{gather*}
把\ding{172}式中的$ x $换成$ b-x $，\ding{173}式中的$ x $
换成$ a-x $，有
\begin{gather*}
    f(a-b+x)=f(a+b-x)  \\
    f(b-a+x)+f(b+a-x)=2c \
...
\begin{figure}[h]   % AI_Function.m
    \centering
    \includegraphics[width=0.95\linewidth]{人工智能激活函数}
\end{figure} 
其中一些函数的导数如下：
\begin{align*}
...
递推关系\ding{172}对应的特征方程为$ x^2=x-1 $，
...
\begin{align*}
    ...
    =\dfrac{(x_1-x_3)(x_2-x_4)}{(x_1-x_4)(x_2-x_3)}
\end{align*}
\textbf{注}：读者是否想过这个性质对于椭圆、抛物线以及实轴虚轴均在坐标轴
上的双曲线上能否成立？答案是都不成立，比如$x^2\pm 2y^2=1,\ y^2=2x$，
很容易选取4个点，让它们的横坐标都是有理数，而纵坐标都是无理数，
那么$ \dfrac{(x_1-x_3)(x_2-x_4)}{(x_1-x_4)(x_2-x_3)} $必然是有理数，
而$ \dfrac{(y_1-y_3)(y_2-y_4)}{(y_1-y_4)(y_2-y_3)} $可以是无理数，
也就是很容易找到两者不相等的反例。
...
\end{gather}
\ding{173}式和\ding{174}式求交集，然后和\ding{172}式求并集，
可得$ a $的范围是：
...
要让曲线逐渐靠近$ x $轴。其图像见图\ref{(6-2x)_(x^2-4)图像}.
\begin{figure}[h] % cubicCurve_2x3_x_x2_4.m
    \centering
    \includegraphics[width=0.6\linewidth]{三次曲线-分子零点在右侧}
    \caption{}
...
再展示几种三次曲线的图像见图\ref{另外3种三次曲线图像}.
\begin{figure}[h] % CubicCurve3type.m
    \centering
    \includegraphics[width=0.95\linewidth]{三次曲线-附加三幅图}
    \caption{}
    \label{另外3种三次曲线图像}
\end{figure} 
