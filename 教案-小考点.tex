...
\item 设$a,b$均为正整数，$(a,b)$代表两者的最大公因数，
$[a,b]$代表两者的最小公倍数，则有
\begin{gather*}
    (a,b)[a,b]=ab
\end{gather*}
比如$a=72,\ b=126$，那么$(a,b)=18,\ [a,b]=504$，
$(a,b)[a,b]=9072=ab$. 

\item 同余的性质：\\
(一) 若 $ a \equiv b \pmod{n} $，且 $ c \equiv d \pmod{n} $，则
\vspace{-3mm}
\begin{enumerate}[label=(\arabic*)]
    \item $ a + c \equiv b + d \pmod{n} $； 
    \item $ ac \equiv bd \pmod{n} $；  
    \item $ ka \equiv kb \pmod{n} $，$ k $ 为任意整数；   
    \item $ a^m \equiv b^m \pmod{n} $，$ m $ 为正整数。
\end{enumerate}  
\vspace{-3mm}
(二) 若 $ ab \equiv ac \pmod{n} $，且 $ (a, n) = 1 $，则 $ b \equiv c \pmod{n} $.

\item 如果一次同余方程$ax\equiv b \pmod{n}$有解，则$(a,n)\mid b$ ；反之，
当$(a,n)\mid b$时，一次同余方程$ax\equiv b \pmod{n}$恰好有$(a,n)$个解。

\item 设$a,b,c$为固定的整数，且$ab\neq 0$. 如果不定方程$ax+by=c$有整数解，
则$(a,b)\mid c$ ；反之，当$(a,b)\mid c$时，不定方程$ax+by=c$有整数解。
类似地，三元一次不定方程$ax+by+cz=d$有整数解的充要条件为
$(a,b,c)\mid d$ ；四元一次不定方程$ax+by+cz+dw=e$有整数解的充要条件为
$(a,b,c,d)\mid e$. 

\item \textbf{孙子定理}：设$a,b,c$为两两互质的正整数，$e,f,g$为任意整数，
则同余方程组
\begin{gather*}
    \begin{dcases}
        x\equiv e \pmod{a} \\
    ...
    \end{dcases}
\end{gather*}
仅有一解：$x\equiv ebcc_1+facc_2+gabc_3 \pmod{abc}$，其中$c_1,c_2,c_3$
分别为满足同余式：$c_1bc\equiv 1\pmod{a} $, 
$ac_2c\equiv 1\pmod{b} $, $abc_3\equiv 1\pmod{c} $的整数。
...
\item 设$ \vec{i},\vec{j} $都是单位向量，这两者既不平行又不垂直，
有非零向量$ \vec{a}=x_1\vec{i}+y_1\vec{j},
...
$ \begin{dcases}
    ...
    \beta=\dfrac{1}{2} \\
    \gamma =-\dfrac{1}{2}
\end{dcases} $，所以$ T=A\sqrt{\dfrac{l}{g}} $. 事实上，通过其它研究手段，
...
\item 无理数的无理数次幂未必是无理数。考虑$\sqrt{2}^{\sqrt{2}}$，
如果这是一个有理数，那我们就找到了一个说明前述结论成立的例子。
如果这是一个无理数，那么$ \Big(\sqrt{2}^{\sqrt{2}}\Big)^{\sqrt{2}}
=\sqrt{2}^2=2$，那我们还是找到了一个说明前述结论成立的例子。
