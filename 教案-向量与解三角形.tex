...
轴正方向的单位向量。两条长竖线在此表示三阶行列式(而非绝对值)，
三阶行列式的定义参见\eqref{三阶行列式的定义}式。向量积的结果仍然是向量。
向量积不满足交换律，即$ \vec{a}\times
\vec{b}\neq \vec{b}\times\vec{a} $，
而是满足反交换律：$ \vec{a}\times \vec{b}=-\vec{b}
\times\vec{a} $. 请读者通过\eqref{三维向量叉乘}式自行验证。
另外还有下式成立：
\begin{gather}\label{向量叉乘分配率}
	(\vec{a}+\vec{b})\times (\vec{c}+\vec{d})=\vec{a}\times \vec{c}+
	\vec{a}\times\vec{d}+\vec{b}\times\vec{c}+\vec{b}\times\vec{d}
\end{gather}
...
其中，$ \begin{vmatrix}
    a_{1} & a_{2} \\
    b_{1} & b_{2}
\end{vmatrix}=a_1b_2-a_2b_1 $代表二阶行列式
(参见\eqref{二阶行列式的定义}式)，
...
\end{align}
实际上，$ \begin{dcases}
   (a_1-a_2 \i)(b_1+b_2 \i)=(a_1b_1+a_2b_2)+(a_1b_2-a_2b_1)\i  \\
   (a_1+a_2 \i)(b_1+b_2 \i)=(a_1b_1-a_2b_2)+(a_1b_2+a_2b_1)\i  
\end{dcases} $，其中$ \i $是虚数单位。
...
\item 如下图，设$\begin{dcases}
    \vec{AD}=r\vec{AB}+s\vec{AC} \\
    \vec{AE}=t\vec{AB}+u\vec{AC} 
\end{dcases}$，
\begin{figure}[H]
    \centering
    \includegraphics[width=0.3\linewidth]{PDF_Picture/二重积分换元公式三角形面积比}
\end{figure}
那么
\begin{align*}
    S_{\Delta ADE}=&\ \dfrac{1}{2}\left|\vec{AD}\times\vec{AE}\right|
    =\dfrac{1}{2}\left|(r\vec{AB}+s\vec{AC})\times
    (t\vec{AB}+u\vec{AC} )\right| \q (\text{利用}\eqref{向量叉乘分配率}\text{式}) \\
    =&\ \dfrac{1}{2}\left|(ru-st)\vec{AB}\times\vec{AC}\right| 
    =|ru-st|\cdot\dfrac{1}{2}\left|\vec{AB}\times\vec{AC}\right| \\
    =&\ |ru-st|\, S_{\Delta ABC} = \begin{Vmatrix}
        r & s \\
        t & u
    \end{Vmatrix} S_{\Delta ABC}
\end{align*}
注意，向量的叉乘满足反交换律，即$ \vec{AB}\times\vec{AC}=-
\vec{AC}\times\vec{AB} $，而且一个向量与自身进行叉乘的结果为零向量，
即$\vec{AB}\times\vec{AB}=\vec{AC}\times\vec{AC}=\vec{0} $. 
$\begin{Vmatrix}
    r & s \\
    t & u
\end{Vmatrix} $包含了两层竖线，内层竖线的含义是二阶行列式，
外层竖线的含义是取绝对值。
理解$\dfrac{S_{\Delta ADE}}{S_{\Delta ABC}}=|ru-st|$，
对于今后理解二重积分的变量代换公式至关重要。
...
\footnote{ 类似于(\ref{三维向量叉乘})式定义的向量积还能适用于七维，
    而无法推广到其它维度。参见 \\
https://zhuanlan.zhihu.com/p/24562904392 }
...
\footnote{Unreal Engine 4(虚幻引擎4，简称UE 4)采用了PhysX和Chaos作为物理引擎，
但UE 5放弃了PhysX，只采用Chaos.}，
...
\item Inigo Quilez是一位计算机图形学专家，2009-2014年，
他在皮克斯(Pixar)动画工作室负责为电影构建程序化布景，进行镜头中的服装设计、建模和着色，以及创建完成这些工作所需的软件/算法和架构，包括底层开发。后来担任过Oculus Story studio的视觉特效总监，目前是Adobe公司的首席研究工程师。他的个人主页是
https://iquilezles.org，该网页上页他发布的一些高质量的技术文章和教学视频，感兴趣的读者可前去学习。下图看起来非常像实拍的照片，然而却是Inigo Quilez用一些并不复杂的函数生成的，生成步骤参见视频《用数学画风景》
\footnote{https://www.bilibili.com/video/BV1Da4y1q78H/}.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\linewidth]{位图/InigoQuilez_Bilibili投稿作品}
\end{figure}
...
\ding{172}若$ A,B,C\in\left(0,\dfrac{\pi}{2}\right] $，
根据正弦定理和正弦函数的单调性，必有“大边对大角”、“大角对大边”。\\
\ding{173}若三角形有一个角是钝角，不妨设为$ C $，根据余弦定理，
$ c^2=a^2+b^2-2ab\cos C $，$ c^2 $同时大于$ a^2 $和$ b^2 $，说明$ c $是最长的边。
反过来，若$ c $是最长的边，$ C $一定是钝角。剩余两个锐角符合\ding{172}中的情形。
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.3\linewidth]{PDF_Picture/三角形中线长公式}
\end{figure}
...
以这些勾股数为边长的直角三角形的周长和面积自然都是整数，以此为基础，
也很容易构造出周长和面积都是整数且不含直角的三角形，
比如把$(6,8,10)$和$(8,15,17)$的等长的
边重合在一起，成为边长为$(10,17,21)$的非直角三角形，其周长和面积也都是整数。
...
\end{align*}
将括号展开后可得：
\begin{align}\label{三角形面积的平方1_16(-u+2v)}
...
在\eqref{三角形面积的平方1_16(-u+2v)}式中，
记$ u=a^4+b^4+c^4,\ v=a^2b^2+b^2c^2+c^2a^2 $. 然后记$ w=a^2+b^2+c^2 $，
那么$ w^2=u+2v $，根据柯西不等式：$ u\geq v $，那么
\begin{gather*}
    w^2=u+2v \geq 3(-u+2v)=48\cdot\dfrac{-u+2v}{16}=48 S_{\Delta ABC}^2
\end{gather*}
开平方后有：
...
\end{gather*}
此不等式显然成立。 等号成立的条件是$a=b$且$C=\dfrac{\pi}{3}$.
所以，当$ a^2+b^2+c^2=4\sqrt{3}S_{\Delta ABC}=
\begin{dcases}
    2\sqrt{3}ab\sin C \\
    2\sqrt{3}bc\sin A \\
    2\sqrt{3}ac\sin B
\end{dcases}$时，$ \Delta ABC $一定是等边三角形。
...
\begin{gather}\label{三角形面积公式汇总}
    ...
    \end{gathered} \right\}
\end{gather}
其中，$ R $是外接圆半径，$ r $是内切圆半径，$ p $为周长的一半。

\item 将余弦定理的三个式子相加：
\begin{align*}
    a^2+b^2+c^2 &=2bc\cos A+2ac\cos B+2ab\cos C \\
    &=8R^2(\sin B\sin C\cos A+\sin A\sin C\cos B+\sin A\sin B\cos C) \\
    &\geq 4\sqrt{3}S_{\Delta ABC}=4\sqrt{3}\cdot 2R^2\sin A\sin B\sin C
\end{align*}
两边约去$ 8R^2 $后可得外森比克不等式的等价形式：
\begin{gather*}
...
\lambda \vec{AB} +\mu \vec{AC} $，则$ \lambda+\mu=n $. 
\begin{figure}[H]
    \centering
...
\item 三角形的三条中线的交点称为\textbf{重心}。
设$ \Delta ABC $的重心为$ G $，三条中线
...
则 $ \begin{dcases}
    \dfrac{\lambda}{2} =\ 1-\mu  \\
    \dfrac{\mu}{2} =\ 1-\lambda
\end{dcases} $，解得$ \lambda=\mu=\dfrac{2}{3} $，即
\begin{gather}\label{重心AG=1_3AB+1_3AC}
    \vec{AG}=\dfrac{2}{3}\vec{AD}+\dfrac{1}{3}\vec{AC}=
    \dfrac{2}{3}\vec{AF}+\dfrac{1}{3}\vec{AB}=
    \dfrac{1}{3}\left(\vec{AB}+\vec{AC}\right)
\end{gather}
已经能说明重心分割中线的比例为1:2. 
所以$ \vec{AG}=2\vec{GE}=\vec{GB}+\vec{GC} $. 
\begin{gather*}
    \vec{GA}+\vec{GB}+\vec{GC}=\vec{0} 
\end{gather*}
\begin{gather}\label{AG·BC表达式}
    \vec{AG}\cdot \vec{BC}=\dfrac{1}{3}\left(\vec{AC}+\vec{AB}\right)\cdot
    \left(\vec{AC}-\vec{AB}\right)=
    \dfrac{1}{3}\left(|\vec{AC}|^2-|\vec{AB}|^2\right)=
    \dfrac{1}{3}\left(b^2-c^2\right)
\end{gather}
再设$ O $为空间中的任意一点，则
\begin{align*}
    \vec{OG}=\vec{OA}+\vec{AG} =&\ 
    \vec{OA}+\dfrac{1}{3}\left(\vec{AB}+\vec{AC}\right) \\
...
\item 在$\Delta ABC$中，$A'$是$AB$边的$n\ (n\geq 2)$等分点中的第$k$个
($1\leq k\leq n-1$，假设最靠近$A$的等分点为第1个)，
$B',\ C'$两点的定义与$A'$类似，
那么$\Delta A'B'C'$的重心与$\Delta ABC$的重心重合。
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{PDF_Picture/三角形三边等分点-无限缩小}
\end{figure}
\textbf{证}：在$\Delta ABC$所在平面内任取一点$O$，那么
\begin{gather*}
    \begin{dcases}
        \vec{OA'}=\dfrac{n-k}{n}\vec{OA}+\dfrac{k}{n}\vec{OB} \\
        \vec{OB'}=\dfrac{n-k}{n}\vec{OB}+\dfrac{k}{n}\vec{OC} \\
        \vec{OC'}=\dfrac{n-k}{n}\vec{OC}+\dfrac{k}{n}\vec{OA} 
    \end{dcases} \\
    \vec{OA'}+\vec{OB'}+\vec{OC'}=\vec{OA}+\vec{OB}+\vec{OC} \\
    \dfrac{1}{3}\left(\vec{OA'}+\vec{OB'}+\vec{OC'}\right)=\vec{OG'}=
    \dfrac{1}{3}\left(\vec{OA}+\vec{OB}+\vec{OC} \right)=\vec{OG}
\end{gather*}
$G'$是$\Delta A'B'C'$的重心，$G$是$\Delta ABC$的重心，
上式说明两者重合。证毕。\\
如果把上述由$\Delta ABC$到$\Delta A'B'C'$的变换称为
“等分点缩小变换”\footnote{这是笔者取的称呼，可能不是一个正式的称呼。}，
那么该变换保持三角形的重心位置不变。
找到“\underline{某些性质或变量在进行某种变换后不变}”是数学研究中最重要
的手段之一，本书的\ref{分式线性变换保交比例题}、\ref{二次方程线性变换}
\footnote{在\ref{二次方程线性变换}中，对于第(1)问，只要$A=1$，即只对二次方程做平移变换，那么判别式就是平移变换中的不变量。对于第(2)问，只要$AD-BC=1$，
那么判别式就是这个特殊的分式线性变换中的不变量。}、\eqref{二次曲线不变量I1}式、
\eqref{二次曲线不变量I2}式、\eqref{二次曲线不变量I3}式都是“不变量”的例子。
...
y\vec{AC} $，则 $ \dfrac{(1-\mu_1)\mu_2}{x}+\dfrac{\mu_1\mu_2}{y}=1 $. 
\begin{figure}[H]
    \centering
...
\end{dcases} $. 所以$ \dfrac{(1-\mu_1)\mu_2}{x}+\dfrac{\mu_1\mu_2}{y}
=\lambda+1-\lambda=1 $. \\
...
\item 三角形的三条高的交点称为\textbf{垂心}。
设$ \Delta ABC $的垂心为$ H $，则$ \vec{HA}\cdot \vec{BC}=
\vec{0}=\vec{HA}\cdot (\vec{HC}-\vec{HB}),\ 
\vec{HA}\cdot \vec{HC}=\vec{HA}\cdot \vec{HB} $，
采用类似方法，可以得到
\begin{gather}\label{垂心性质三个连等式向量形式}
    \vec{HA}\cdot \vec{HB}=
    \vec{HA}\cdot \vec{HC}=\vec{HB}\cdot \vec{HC} 
\end{gather}
因为$\begin{dcases}
    \vec{HA}\cdot \vec{HB}=-|HB|\cdot |HD| \\
    \vec{HB}\cdot \vec{HC}=-|HC|\cdot |HE| \\
    \vec{HC}\cdot \vec{HA}=-|HA|\cdot |HF| 
\end{dcases}$，所以\eqref{垂心性质三个连等式向量形式}式
又能以标量形式写成
\begin{gather}\label{垂心性质三个连等式标量形式}
    |HA|\cdot |HF|=|HB|\cdot |HD|= |HC|\cdot |HE|
\end{gather}
\begin{figure}[H]
    \centering
    ...
    \label{三角形垂心第一张图}
\end{figure}
在上图中，设$O$为$ \Delta ABC $的外接圆圆心(外心)，连接$BO$并延长，交外接圆于$K$点，
连接$AK, CK$. 因为$\angle EAC+\angle CAK =\dfrac{\pi}{2}=\angle EAC+\angle ECA $，
所以$\angle CAK=\angle ECA $，$HC//AK$(内错角相等)。又因为
$\angle AFC=\angle KCF=\dfrac{\pi}{2}$，所以$AH//CK$(同时垂直于$BC$)，
四边形$AHCK$为平行四边形，设$R$代表外接圆半径，
那么$|AH|=|KC|=2R|\cos\angle BKC|=2R|\cos A|$
(这里的$A$代表$\angle BAC$)。同理可得：$|BH|=2R|\cos B|,\ |CH|=2R|\cos C|$.
...
+\angle BHC =\pi $($\angle BHC$和$\angle EHD$是对顶角)，同理可得：
$ \angle BCA+\angle BHA =\pi,\ \angle ABC+\angle AHC =\pi $. 

\item 在图\ref{三角形垂心第一张图}中，设$|AB|=c,\ |BC|=a,\ |CA|=b$，那么
\begin{align*}
    \vec{AF}=\dfrac{|FC|}{|BC|}\vec{AB}+\dfrac{|BF|}{|BC|}\vec{AC}
    =&\ \dfrac{b\cos C}{a}\vec{AB}+\dfrac{c\cos B}{a}\vec{AC} \\
    =&\ \dfrac{\sin B\cos C}{\sin A}\vec{AB}+
    \dfrac{\sin C\cos B}{\sin A}\vec{AC}
\end{align*}
再利用上一条的结果，下面之所以写成$\vec{AH}=\pm \dfrac{|AH|}{|AF|}\vec{AF}$，
是因为$\vec{AH}$与$\vec{AF}$可能同向也可能反向(或者说垂心可能在三角形内部
也可能在三角形外部)。
\begin{align*}
    \vec{AH}=\pm \dfrac{|AH|}{|AF|}\vec{AF}
    =&\ \dfrac{2R\cos A}{(2R\sin C)\sin B}
    \left(\dfrac{\sin B\cos C}{\sin A}\vec{AB}+
          \dfrac{\sin C\cos B}{\sin A}\vec{AC}\right) \\
    =&\ \dfrac{1}{\tan A\tan C}\vec{AB}+
    \dfrac{1}{\tan A\tan B}\vec{AC}
\end{align*}
两边同乘$\tan A\tan B\tan C$，有
\begin{gather*}
    \tan A\tan B\tan C \vec{AH}=\tan B(\vec{HB}-\vec{HA})
    +\tan C(\vec{HC}-\vec{HA})
\end{gather*}
又因为$\tan(A+B)=\dfrac{\tan A+\tan B}{1-\tan A\tan B}=-\tan C$，
所以$ \tan A\tan B\tan C=\tan A+\tan B+\tan C $，于是
\begin{gather*}
    -(\tan A+\tan B+\tan C)\vec{HA}=\tan B(\vec{HB}-\vec{HA})
    +\tan C(\vec{HC}-\vec{HA})
\end{gather*}
整理后可得：
\begin{gather*}
    \tan A\cdot \vec{HA}+\tan B\cdot \vec{HB}
    +\tan C\cdot\vec{HC}=\vec{0}
\end{gather*}

\item 三角形的三条内角平分线的交点称为\textbf{内心}，因为它是内切圆的圆心。
设$ \Delta ABC $的内心为$ I $，若$ \vec{AI}=\lambda\vec{AB}+\mu\vec{AC} $，
...
\textbf{注}\ 由$ \tan\left(\dfrac{A}{2}+\dfrac{B}{2}\right)=
\dfrac{\tan \dfrac{A}{2}+\tan \dfrac{B}{2}}
{1-\tan \dfrac{A}{2}\tan \dfrac{B}{2}}=
\tan\left(\dfrac{\pi}{2}-\dfrac{C}{2}\right)=
...
设$ \vec{AI}=\rho\left(\dfrac{\vec{AB}}{|
    \vec{AB}|}+\dfrac{\vec{AC}}{|\vec{AC}|}\right) $，
    那么$ |\vec{AI}|^2=2\rho^2(1+\cos A) $
    (因为$\dfrac{\vec{AB}}{|\vec{AB}|},\ \dfrac{\vec{AC}}{|\vec{AC}|}$
    的长度都是1). 又有$ |\vec{AI}|=\dfrac{r}{\sin \dfrac{A}{2}} $，所以
\begin{gather*}
    |\vec{AI}|^2=\dfrac{r^2}{\sin^2 \dfrac{A}{2}}=
    \dfrac{2r^2}{1-\cos A}= 2\rho^2(1+\cos A) \\
    \rho=\dfrac{r}{\sin A}=\dfrac{1}{\sin A}\sqrt{\dfrac{(p-a)(p-b)(p-c)}{p}} 
\end{gather*}
\begin{align*}
    \vec{AI}\cdot\vec{BC}=&\ \rho\left(\dfrac{
        \vec{AB}}{|\vec{AB}|}+\dfrac{\vec{AC}}{|\vec{AC}|}\right)
    \cdot(\vec{AC}-\vec{AB}) \\
    =&\ \dfrac{\rho}{|\vec{AB}|\cdot |\vec{AC}|}(
    \vec{AB}\cdot \vec{AC} |\vec{AC}|-\vec{AB}^2|\vec{AC}|+
    \vec{AC}^2|\vec{AB}|-\vec{AB}\cdot \vec{AC}|\vec{AB}|) \\
    =&\ \rho(1+\cos A)\left(|\vec{AC}|-|\vec{AB}|\right) \\
...
\begin{gather*}
    |AI|\cdot|BI|\cdot|CI|=\frac{r^3}{\sin\dfrac{A}{2}\sin\dfrac{B}{2}
        \sin\dfrac{C}{2}}=4Rr^2
\end{gather*}

\item 三角形的三条边的垂直平分线的交点称为\textbf{外心}，
因为它是外接圆的圆心。$ \Delta ABC $的外心为$ O $，
设$ \vec{AO}=\eta\vec{AB}+\kappa\vec{AC} $，那么 
\begin{gather*}
\begin{dcases}
    \vec{AO}\cdot\vec{AB}=\dfrac{1}{2}|AB|^2
    =\eta |AB|^2+\kappa |AB|\cdot |AC|\cdot\cos A \\
    \vec{AO}\cdot\vec{AC}=\dfrac{1}{2}|AC|^2 
    =\eta |AB|\cdot |AC|\cdot\cos A+\kappa |AC|^2
\end{dcases}
\end{gather*}
两式相减，有
\begin{gather*}
    \vec{AO}\cdot (\vec{AC}-\vec{AB})=\vec{AO} \cdot \vec{BC}=
    \dfrac{1}{2}|\vec{AC}|^2-\dfrac{1}{2}|\vec{AB}|^2=
    \dfrac{1}{2}(\vec{AC}+\vec{AB})\cdot(\vec{AC}-\vec{AB})
\end{gather*}
注意，这个等式说明$\vec{AO}$与$\dfrac{1}{2}(\vec{AC}+\vec{AB})$
在$\vec{BC}$上的投影的长度相等(请读者自己画图思考)，而不是说明
$\vec{AO}=\dfrac{1}{2}(\vec{AC}+\vec{AB})$.
\begin{gather*}
\begin{dcases}
    \eta |AB|+\kappa |AC|\cdot\cos A=\dfrac{1}{2}|AB| \\
    \eta |AB|\cdot \cos A+\kappa |AC|=\dfrac{1}{2}|AC| 
\end{dcases}\q \Rightarrow \q
\begin{dcases}
...
\begin{gather*}
    S_A:S_B:S_C=\tan A:\tan B:\tan C \\ 
    \tan A\cdot \vec{OA}+\tan B\cdot \vec{OB}
...
对于锐角三角形$ ABC $，有
\begin{align}
    \sin A +\sin B +\sin C &>\dfrac{2}{\pi}(A+B+C)=2 \label{sinA+sinB+sinC大于2} \\
    \cos A +\cos B +\cos C &>3-\dfrac{2}{\pi}(A+B+C)=1 \label{cosA+cosB+cosC大于1}
\end{align}
...
\textbf{解}\ 根据(\ref{垂心性质三个连等式向量形式})式，可设 $ \vec{HA}\cdot
\vec{HB}=\vec{HA}
...
\textbf{方法二}\ 基本不等式：
$ x+y=(x+y)\left (\dfrac{1}{x}+\dfrac{4}{y}\right)=5+\dfrac{y}{x}+\dfrac{4x}{y}
\geq 5+ 2\sqrt{\dfrac{y}{x}\cdot \dfrac{4x}{y}}=9 $. \\
\textbf{方法三}\ 柯西不等式：$ x+y=(x+y)\left(\dfrac{1}{x}+\dfrac{4}{y}\right)
...
\begin{gather}\label{点列交比与线束交比相等}
...
\begin{align}
    ...
    =\dfrac{|PC|}{\sin\angle PAC} \tag*{\ding{172}} \\
    & \dfrac{|BD|}{\sin\angle BPD}=\dfrac{|PB|}{\sin\angle PDB} 
    =\dfrac{|PD|}{\sin\angle PBD} \tag*{\ding{173}}\\
    & \dfrac{|AD|}{\sin\angle APD}=\dfrac{|PD|}{\sin\angle PAC}
    =\dfrac{|PA|}{\sin\angle PDB} \tag*{\ding{174}}\\    
    & \dfrac{|BC|}{\sin\angle BPC}=\dfrac{|PC|}{\sin\angle PBC}
    =\dfrac{|PB|}{\sin\angle PCB} \tag*{\ding{175}}
\end{align}
...
\end{align*}
上面的计算使用了\ding{172}$ \sim $\ding{175}式的中间一列，
...
设$ \vec{OC}=\lambda\vec{OA}+\mu\vec{OB} $，求：\\
(1) $ \lambda $和$\mu $.\q \q (2) 当$t$取何值时，$\lambda+\mu$取得最大值？
\begin{figure}[H]
    \centering
    \includegraphics[width=0.25\linewidth]{单位圆上3个向量v1v2合成v四元数背景}
\end{figure} 
\textbf{解}\ (1) 过$ C $ 点作$ OB $的平行线，与$ OA $交于$ D $点，
则$ \vec{OC}=\vec{OD}+\vec{DC}=\lambda\vec{OA}+\mu\vec{OB} $，
在$ \Delta ODC $中应用正弦定理，
\begin{figure}[H]
    \centering
    \includegraphics[width=0.25\linewidth]{单位圆上3个向量v1v2合成v四元数背景-含辅助线}
...
(2) \textbf{方法一}：连接$AB$，设$AB$与$OC$交于$E点$，那么
$ \vec{OE}=\dfrac{1}{\lambda+\mu}\vec{OC} $，
$\lambda+\mu=\dfrac{|\vec{OC}|}{|\vec{OE}|}=\dfrac{1}{|\vec{OE}|}$，
当$OE\perp AB$时，$|\vec{OE}|$取得最小值$ \cos\dfrac{\theta}{2} $，
而$\lambda+\mu$取得最大值$ \dfrac{1}{\cos\frac{\theta}{2}} $. \\
\textbf{方法二}：
\begin{align*}
    \lambda+\mu=&\ \dfrac{\sin((1-t)\theta)+\sin(t\theta)}{\sin\theta}\\
    =&\ \dfrac{\sin\theta\cos(t\theta)+
        [1-\cos\theta]\sin(t\theta)}{\sin\theta}\\
    =&\ \dfrac{2\sin\frac{\theta}{2}\cos\frac{\theta}{2}\cos(t\theta)+
        2\sin^2\frac{\theta}{2}\sin(t\theta)}{2\sin\frac{\theta}{2}
        \cos\frac{\theta}{2}} \\
    =&\ \dfrac{\cos\frac{\theta}{2}\cos(t\theta)+
        \sin\frac{\theta}{2}\sin(t\theta)}{\cos\frac{\theta}{2}} 
    =\dfrac{\cos(\frac{\theta}{2}-t\theta)}{\cos\frac{\theta}{2}}
    \leq\dfrac{1}{\cos\frac{\theta}{2}}
\end{align*}
当$t=\dfrac{1}{2}$时，等号成立，$\lambda+\mu$取得最大值
$ \dfrac{1}{\cos\frac{\theta}{2}} $. \\
\textbf{方法三}：\begin{gather*}
    \vec{OC}^2=1=\left(\lambda\vec{OA}+\mu\vec{OB}\right)^2
    =\lambda^2+\mu^2+2\lambda\mu\cos\theta 
\end{gather*}
设$s=\lambda+\mu $，则
\begin{align*}
    s^2=(\lambda+\mu)^2=&\ (\lambda^2+\mu^2)+2\lambda\mu \\
    =&\ (1-2\lambda\mu\cos\theta)+2\lambda\mu=
    1+2\lambda\mu(1-\cos\theta) \\
    =&\ 1+2\lambda(s-\lambda)(1-\cos\theta) 
\end{align*}
移项后有$2(1-\cos\theta)\lambda^2-2s(1-\cos\theta)\lambda+s^2-1=0$，
这个关于$\lambda$的二次方程必然有实数根，判别式非负，即
\begin{gather*}
    \Delta=4s^2(1-\cos\theta)^2-8(1-\cos\theta) (s^2-1)\geq 0  \\
    s^2\leq \dfrac{2}{1+\cos\theta} 
    =\dfrac{1}{\cos^2\frac{\theta}{2}}
\end{gather*}
等号成立的条件是$\lambda=\mu=\dfrac{1}{2\cos\frac{\theta}{2}}$，
$t=\dfrac{1}{2}$，
所以$s=\lambda+\mu $的最大值为$ \dfrac{1}{\cos\frac{\theta}{2}} $. \\
...
本题涉及的知识点包括：向量、正弦定理、三角函数、不等式、
罗必塔法则、四元数插值等，可谓本书中最精彩的例题之一了。
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.6\linewidth]{"1_7面积占比 向量题"}
\end{figure}
\\
\textbf{方法一}\ 向量法：\\
\begin{gather*}
...
\end{gather*}
\begin{gather}
    t^2\pm(\tan\theta)(x+y)t-xy=0   \tag*{\ding{172}}
\end{gather}
直接解方程，就能得到$ t $. 
现在让我们站在命题人的角度来看方程\ding{172}，我们希望解出的$ t $
是有理数(不含有根式)，最好还是整数，以减小计算量。为满足这个要求，该如何选定$ x,y $呢？
让我们从韦达定理的角度考虑。\\
(一) 方程\ding{172}的两根为$ x,-y $，那么
\begin{gather}
    x-y=\pm(\tan\theta)(x+y)  \tag*{\ding{173}}
\end{gather}
只要让$ \tan\theta $是有理数，那么当$ x $是有理数时，$ y $也必然是有理数。\\
(二) 方程\ding{172}的两根为$ 1,-xy $，那么
\begin{gather}
    1-xy=\pm(\tan\theta)(x+y) \tag*{\ding{174}}
\end{gather}
只要让$ \tan\theta $是有理数，那么当$ x $是有理数时，$ y $也必然是有理数。\\
(三) 方程\ding{172}的两根为$ \sqrt{x},-\sqrt{x}y $，那么
\begin{gather}
    \sqrt{x}(1-y)=\pm(\tan\theta)(x+y) \tag*{\ding{175}}
\end{gather}
只要让$ \tan\theta $是有理数，那么当$ \sqrt{x} $是有理数时，$ y $也必然是有理数。\\
\\
\textbf{方法二}\ $ \cos\theta=\dfrac{tx+ty}{\sqrt{x^2+t^2}\sqrt{y^2+t^2}} $，整理得：
\begin{align}
    t^4+\left[x^2+y^2-\dfrac{1}{\cos^2\theta}(x+y)^2\right]
    t^2+x^2y^2=0  \tag*{\ding{176}}
\end{align}
这是关于$ t^2 $的二次方程。\\
(一) 方程\ding{176}的两根为$ x^2,y^2 $，那么
\begin{gather*}
...
\end{gather*}
这与\ding{173}式等价。\\
(二) 方程\ding{176}的两根为$ 1,x^2y^2 $，那么
\begin{gather*}
...
\end{gather*}
这与\ding{174}式等价。\\
(三) 方程\ding{176}的两根为$ x,xy^2 $，那么
\begin{gather*}
    x+xy^2=\dfrac{1}{\cos^2\theta}(x+y)^2-(x^2+y^2) \\
    x+xy^2-2xy=\dfrac{1}{\cos^2\theta}(x+y)^2-(x^2+y^2+2xy) 
\end{gather*}
\begin{gather}
    x(1-y)^2=(\tan\theta)^2(x+y)^2  \tag*{\ding{177}}
\end{gather}
这与\ding{175}式等价。列举一些整数解的情形：$ \tan\theta=3 $，
\ding{177}式有$ (1,-2),(4,-14) $等整数解；
$ \tan\theta=4 $，\ding{177}式有$ (4,-9),(9,-39) $等整数解；
$ \tan\theta=5 $，\ding{177}式有$ (9,-24),(16,-84) $等整数解。
...
\begin{gather*}
    ...
    =2(a^4+b^4+c^4)-2(a^2b^2+b^2c^2+c^2a^2) \tag*{\mycircled{7}}
\end{gather*}
由 \mycircled{1}+\mycircled{2}+\mycircled{3} 可得
\begin{align*}
    2(x^2+y^2+z^2)+(xy+yz+zx)=a^2+b^2+c^2  \tag*{\mycircled{8}}
\end{align*}
记
\begin{align*}
    & s_1=x+y+z \\
...
\item 如图\ref{两轴机械臂逆解-无辅助线}，设$|OA|=l_1,\ |AB|=l_2$，
$OA$与$x$轴正方向的夹角为$\theta_1$，$AB$与$OA$的延长线
$AC$的夹角为$\theta_2$($\theta_1,\theta_2$均以逆时针
方向为正)，且$\theta_1>0,\ \theta_2>0 $，
$\theta_1+\theta_2<\dfrac{\pi}{2}$. 又设$B$点坐标为$(u,v)$，
且$u>0,v>0$，用只含$l_1,l_2,u,v$的式子表示$\cos\theta_1$和
$\cos\theta_2$. 
\begin{figure}[H]
    \centering
    \includegraphics[width=0.3\linewidth]{PDF_Picture/两轴机械臂逆解(无辅助线)}
    \caption{}
    \label{两轴机械臂逆解-无辅助线}
\end{figure}
\textbf{解}：如图\ref{两轴机械臂逆解-有辅助线}，连接$O,B$，
过$B$点作$OC$的垂线，垂足为$D$，过$B$点作$x$轴的垂线，垂足为$E$，
\begin{figure}[H]
    \centering
    \includegraphics[width=0.3\linewidth]{PDF_Picture/两轴机械臂逆解(有辅助线)}
    \caption{}
    \label{两轴机械臂逆解-有辅助线}
\end{figure}
\vspace{-10mm}
\begin{align*}
    u=&\ l_1\cos\theta_1+l_2\cos(\theta_1+\theta_2) 
    \tag*{\ding{172}}  \\
    v=&\ l_1\sin\theta_1+l_2\sin(\theta_1+\theta_2)    
    \tag*{\ding{173}}
\end{align*}
以上两式平方后相加，有
\begin{align*}
    u^2+v^2=&\ l_1^2+l_2^2+2l_1l_2[\cos\theta_1\cos(\theta_1+
    \theta_2)+\sin\theta_1\sin(\theta_1+\theta_2)] \\
    =&\ l_1^2+l_2^2+2l_1l_2\cos(\theta_1-(\theta_1+\theta_2)) \\
    =&\ l_1^2+l_2^2+2l_1l_2\cos\theta_2 \q (=|OB|^2)  
\end{align*}
于是，
\begin{gather*}
    \cos\theta_2=\dfrac{|OB|^2-l_1^2-l_2^2}{2l_1l_2}
    =\dfrac{u^2+v^2-l_1^2-l_2^2}{2l_1l_2} \tag*{\ding{174}}
\end{gather*}
上式实际上相当于在$\Delta OAB$中使用余弦定理。然后就能算出
$\sin\theta_2=\sqrt{1-\cos^2\theta_2}$. 
\ding{172}和\ding{173}可以改写为
\begin{align*}
    u=&\ (l_1+l_2\cos\theta_2)\cos\theta_1
    -(l_2\sin\theta_2)\sin\theta_1  \tag*{\ding{175}}  \\
    v=&\ (l_1+l_2\cos\theta_2)\sin\theta_1
    +(l_2\sin\theta_2)\cos\theta_1    \tag*{\ding{176}}
\end{align*}
用$\text{\ding{175}}\times (l_1+l_2\cos\theta_2)+
\text{\ding{176}}\times(l_2\sin\theta_2)$可得：
\begin{align*}
    u(l_1+l_2\cos\theta_2)+vl_2\sin\theta_2=&\ 
    (l_1+l_2\cos\theta_2)^2\cos\theta_1+
    (l_2\sin\theta_2)^2\cos\theta_1 \\
    =&\ (l_1^2+l_2^2+2l_1l_2\cos\theta_2)\cos\theta_1=
    (u^2+v^2)\cos\theta_1 
\end{align*}
\begin{gather*}
    \cos\theta_1=\dfrac{u(l_1+l_2\cos\theta_2)+
        vl_2\sin\theta_2}{u^2+v^2} \tag*{\ding{177}}
\end{gather*}
下面展示另一种思考方法。
\begin{align*}
    \cos\theta_1=&\ \cos(\angle BOE-\angle BOD)  \\
    =&\ \cos\angle BOE\cos\angle BOD+
    \sin\angle BOE\sin\angle BOD \\
    =&\ \dfrac{u}{|OB|}\cdot \dfrac{|OD|}{|OB|} +
    \dfrac{v}{|OB|}\cdot \dfrac{|BD|}{|OB|} \\
    =&\ \dfrac{u(l_1+l_2\cos\theta_2)+
        vl_2\sin\theta_2}{u^2+v^2}
\end{align*}
只要把\ding{174}式以及$\sin\theta_2=\sqrt{1-\cos^2\theta_2}$
代入上式，就能把$\cos\theta_1$用只含$l_1,l_2,u,v$的式子表示。\\
\textbf{注}：题目中之所以对$\theta_1,\theta_2,u,v$的取值范围都做了
约束，是为了减少分类讨论的麻烦。比如，假如不限制$ \theta_2 $的范围，
那么就得考虑$\sin\theta_2=\pm\sqrt{1-\cos^2\theta_2}$的根号前
是取正号还是负号了。本题是学习机械臂运动控制的入门题目，比如
最简单的三轴SCARA(Selective Compliance Assembly Robot Arm，
见图\ref{SCARA机械臂})的运动控制就要用到\ding{174}和\ding{177}式。
也就是在直角坐标系中给定了位置$(u,v)$，然后求解出两个关节的转角
$\theta_1,\theta_2$. 给定关节转角，计算直角坐标，称为“正运动学”；
给定直角坐标，计算关节转角，称为“逆运动学”。
\begin{figure}[H]
    \centering
    \includegraphics[width=0.3\linewidth]{位图/SCARA机械臂}
    \caption{}
    \label{SCARA机械臂}
\end{figure}
...
$\begin{dcases}
    ...
    y^2+z=b \\
    z^2+x=c 
\end{dcases}$，其中$x,y,z$是未知数，$a,b,c$是常数，
通过消元法求出$x$所满足的8次方程。
...
\item $D$是$\Delta ABC$内部的一点，且$\vec{AD}=\dfrac{2}{5}\vec{AB}
+\dfrac{1}{5}\vec{AC}$，求$\dfrac{S_{\Delta BDC}}{S_{\Delta BDA}}$.
...
$s_2=x^2+y^2+z^2=\dfrac{1}{2}\left(w-\sqrt{\dfrac{-u+2v}{3}}\right)
...
1979年高考数学有这样一题：已知$(z-x)^2-4(x-y)(y-z)=0$，
求证：$x,y,z$成等差数列。解决方法与上面类似，先去括号，
再尝试因式分解，
\begin{gather*}
    x^2-2xz+z^2-4(xy-xz-y^2+yz)=0  \\
    x^2+z^2+4y^2+2xz-4xy-4yz=0 \\
    (x+z-2y)^2=0
\end{gather*}
另一种解法是把题目所给等式看成一个二次方程的判别式，比如
\begin{gather*}
    (x-y)t^2+(z-x)t+(y-z)=0 \tag*{\ding{172}}
\end{gather*}
或者
\begin{gather*}
    t^2+(z-x)t+(x-y)(y-z)=0 \tag*{\ding{173}}
\end{gather*}
容易验证，$t=1$是方程\ding{172}的一个根，根据求根公式$t=-\dfrac{z-x}{2(x-y)}=1$，
那么$x-z=2(x-y)$，$x+z=2y$. 类似的，$t=x-y$是方程\ding{173}的一个根，
根据求根公式$t=-\dfrac{z-x}{2}=x-y$，那么$x-z=2(x-y)$，$x+z=2y$. 
...
\item 条件可变形成
\begin{align}
    2\vec{HA}+3\vec{HB} +5\vec{HC}=\vec{0} \tag*{\ding{172}}
\end{align}
设 $ \vec{HA}\cdot \vec{HB}=\vec{HA}
\cdot\vec{HC}=\vec{HB}\cdot \vec{HC}=t<0 $，分别用
$ \vec{HB},\vec{HC} $与\ding{172}式做数量积，
...
\item 延长$AD$交$BC$于$E$点，
\begin{figure}[H]  
    \centering
    \includegraphics[width=0.3\linewidth]{PDF_Picture/面积比值题目3_5}
\end{figure}
$\vec{AD}=\dfrac{2}{5}\vec{AB}
+\dfrac{1}{5}\vec{AC}=\dfrac{3}{5}\left(\dfrac{2}{3}\vec{AB}
+\dfrac{1}{3}\vec{AC}\right)=\dfrac{3}{5}\vec{AE}$，
$\dfrac{S_{\Delta BDE}}{S_{\Delta CDE}}=\dfrac{|BE|}{|CE|}=
\dfrac{1}{2}$，设$S_{\Delta BDE}=S$，那么$S_{\Delta CDE}=2S$.
因为$\dfrac{S_{\Delta BDA}}{S_{\Delta BDE}}=\dfrac{|AD|}{|DE|}=
\dfrac{3}{2}$，所以$ S_{\Delta BDA}=\dfrac{3}{2}S $，
于是，$\dfrac{S_{\Delta BDC}}{S_{\Delta BDA}}=
\dfrac{S+2S}{\frac{3}{2}S}=2$.
