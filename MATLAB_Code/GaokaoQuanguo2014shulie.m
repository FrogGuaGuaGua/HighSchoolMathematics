...
legend('$y=\ln(1+x)$', '$y = x$', '迭代路径', 'Interpreter','latex', 'location','northwest','FontSize', 13);
grid off;
box on
% axis equal;
xlim([-0.2, 1.1]);
...
fplot(@(x) 2/(x+1),'k-','LineWidth', 0.8)
plot([-10,100],[0,0],'k-.')
legend('$a_n$', '$y =\frac{2}{x+1}$','', 'Interpreter','latex', 'location','northeast','FontSize', 14);
